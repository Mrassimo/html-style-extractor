...
Copy this prompt along with the analysis above:

Help me rebuild the exact same UI design as a single HTML file for "${data.pageTitle}".

I've analyzed ${data.pageUrl} and extracted the complete design system. Use the analysis data above as your primary source of truth to create an exact replica.
...
- Include Google Fonts or other font imports from the head content
- Fully functional and interactive
- Responsive design matching the original
- Semantic HTML5 elements

**For Visual Context:**
Upload the ${pageCount} screenshots separately for visual reference. The analysis data provides the technical specifications.

### Step 2: Generate Style Guide
Once replication is complete, use this prompt:

Based on the HTML you created and the original analysis, generate a comprehensive style guide documentation covering:
...
- Visual details (shadows, borders, transitions)

Save as "style-guide.md"
...
${data.screenshots.length > 0 ?
  data.screenshots.map((shot, i) => `${i + 1}. ${shot.label}`).join('\n') +
  '\n\n*Note: Use the "Copy Screenshots" button to get URLs for AI upload*' :
  'No screenshots captured for this analysis'
}

---

## 📦 Complete Data Package
For the complete data including all CSS rules and assets, use the "Download Package" feature to get a ZIP file with:
- Full analysis documentation
- All screenshot image files
- Complete CSS rules from all stylesheets
- HTML structure
- AI prompt templates
- README with usage instructions

*Generated by HTML Style Extractor for LLMs*`;
};
